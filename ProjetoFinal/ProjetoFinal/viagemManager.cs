...
namespace ProjetoFinal
{
    ...
    {
        private int? _viagemIdEmEdicao = null;
        ...
        private ComboBox _cb_situacao; // substitui o TextBox antigo
        private DataGridView _grid;
        private Label _lbl_cancelar_viagem;
        private Label _lbl_salvar_viagem;
        ...
        public ViagemManager(
            ...
            ComboBox cb_situacao,
            DataGridView grid,
            Label lbl_cancelar_viagem,
            Label lbl_salvar_viagem)
        {
            ...
            _cb_situacao = cb_situacao;
            _grid = grid;
            _lbl_cancelar_viagem = lbl_cancelar_viagem;
            _lbl_salvar_viagem = lbl_salvar_viagem;
        }
        ...
        public void CarregarCombos()
        {
            ...
            CarregarVeiculos();
            CarregarRotas();
            // não precisa carregar situações — já vem do form
        }
        ...
        public void Salvar(int? viagemId = null)
        {
            ...
            if (_cb_situacao.SelectedIndex == -1)
            {
                MessageBox.Show("Selecione a situação da viagem.", "Atenção", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                _cb_situacao.Focus();
                return;
            }

            string situacao = _cb_situacao.SelectedItem.ToString();
            ...
            try
            {
                using (var conn = Database.GetConnection())
                {
                    ...
                    using (var cmd = new SQLiteCommand(sql, conn))
                    {
                        ...
                        MessageBox.Show(viagemId == null
                            ? "Viagem cadastrada com sucesso!"
                            : "Viagem atualizada com sucesso!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    ...
                    using (var cmd = new SQLiteCommand(sql, conn))
                    {
                        ...
                        using (var reader = cmd.ExecuteReader())
                        {
                            if (reader.Read())
                            {
                                ...
                                if (_cb_situacao.Items.Contains(situacao))
                                    _cb_situacao.SelectedItem = situacao;
                                else
                                    _cb_situacao.Items.Add(situacao);

                                _lbl_cancelar_viagem.Text = "Cancelar";
                            }
                            else
                            {
        ...
        public void CancelarEdicao()
        {
            ...
            LimparCampos();
            _viagemIdEmEdicao = null;
            _lbl_cancelar_viagem.Text = "Excluir";
            _lbl_salvar_viagem.Text = "Incluir";
        }

        public void AtualizarGrid() => Consultar();
